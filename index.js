export default {
  async fetch(request, env) {
    ...
    // --- 1. LOGOUT LOGICA (NIEUW & VERBETERD) ---
    if (url.searchParams.has('logout')) {
      return new Response('Uitloggen...', {
        status: 302,
        headers: {
    ...
    const browserRows = browsers.results.map(b => {
      ...
      return `<div class="mb-3 text-sm"><div class="flex justify-between items-center mb-1"><span>${b.browser}</span><span class="text-gray-500">${b.count}</span></div><div class="h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full"><div class="h-1.5 bg-blue-500 rounded-full" style="width:${percentage}%"></div></div></div>`;
    }).join('');
    ...
    <body class="bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 p-4 md:p-8 font-sans">
      <div class="max-w-6xl mx-auto">
        <header class="flex justify-between items-center mb-10 pb-4 border-b border-gray-200 dark:border-gray-800">
          <h1 class="text-xl font-bold tracking-tight">PRO ANALYTICS</h1>
          <a href="/?logout=true" class="text-xs font-medium text-red-500 hover:text-red-600 transition-colors uppercase tracking-widest">Uitloggen</a>
        </header>

        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div class="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-800"><h3 class="text-gray-500 text-xs mb-2">Views</h3><p class="text-3xl font-bold text-blue-500 font-mono">${totalViews}</p></div>
          <div class="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-800"><h3 class="text-gray-500 text-xs mb-2">Uniek</h3><p class="text-3xl font-bold font-mono">${uniqueUsers}</p></div>
          <div class="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-800"><h3 class="text-gray-500 text-xs mb-2">Top Pagina</h3><p class="text-sm font-semibold truncate text-blue-500">${topPage.url}</p></div>
          <div class="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-800"><h3 class="text-gray-500 text-xs mb-2">Top Land</h3><p class="text-xl font-bold">${topCountry.country}</p></div>
        </div>

        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800 mb-8"><canvas id="mainChart" height="100"></canvas></div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800">
            <h3 class="text-gray-400 text-xs font-bold uppercase mb-4 tracking-widest">Landen</h3>
            ${countries.results.slice(0, 8).map(c => '<div class="flex justify-between py-1.5 border-b dark:border-gray-800 text-sm"><span>'+c.country+'</span><span class="font-mono text-blue-500 font-bold">'+c.count+'</span></div>').join('')}
          </div>
          <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-gray-200 dark:border-gray-800">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden text-sm">
          <table class="w-full text-left">
            <thead class="bg-gray-100 dark:bg-gray-800 text-gray-400 text-[10px] uppercase tracking-widest">
              <tr><th class="p-3">Paginapad</th><th class="p-3 text-right">IP-adres</th></tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              ${recent.results.map(r => `<tr><td class="p-3 text-blue-500 font-mono text-xs font-medium">${r.url}</td><td class="p-3 font-mono text-right text-gray-500">${r.ip}</td></tr>`).join('')}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <script>
        new Chart(document.getElementById('mainChart'), {
          ...
          options: {
            ...
            scales: {
              y: { grid: { color: (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) ? '#111' : '#f3f4f6' }, beginAtZero: true },
