      ...
      return new Response(`
        <html>
          <body style="display:flex;justify-content:center;align-items:center;height:100vh;background:#000;color:#fff;font-family:sans-serif;margin:0;">
            <form action="/" method="GET" style="background:#111;padding:2rem;border-radius:15px;border:1px solid #333;width:300px;text-align:center;">
          ...
          <div class="bg-gray-900 p-6 rounded-2xl border border-gray-800 text-sm">
            ...
            ${countries.results.map(c => `
              <div class="flex justify-between border-b border-gray-800 py-1">
                <span>${c.country}</span><span class="text-blue-400 font-mono">${c.count}</span>
              </div>`).join('')}
          </div>
        </div>
            ...
            <tbody>
              ${stats.results.map(r => `
                <tr class="border-t border-gray-800">
                  <td class="p-4 text-blue-300 font-mono">${r.url}</td>
                  <td class="p-4 text-right font-bold">${r.views}</td>
                </tr>`).join('')}
            </tbody>
          </table>
        </div>
